...
static int ngx_stream_lua_ngx_worker_pid(lua_State *L);
static int ngx_stream_lua_ngx_worker_id(lua_State *L);
static int ngx_stream_lua_ngx_worker_count(lua_State *L);


void
ngx_stream_lua_inject_worker_api(lua_State *L)
{
    lua_createtable(L, 0 /* narr */, 4 /* nrec */);    /* ngx.worker. */

    lua_pushcfunction(L, ngx_stream_lua_ngx_worker_exiting);
    lua_setfield(L, -2, "exiting");

    lua_pushcfunction(L, ngx_stream_lua_ngx_worker_pid);
    lua_setfield(L, -2, "pid");

    lua_pushcfunction(L, ngx_stream_lua_ngx_worker_id);
    lua_setfield(L, -2, "id");

    lua_pushcfunction(L, ngx_stream_lua_ngx_worker_count);
...
static int
ngx_stream_lua_ngx_worker_exiting(lua_State *L)
{
...
static int
ngx_stream_lua_ngx_worker_pid(lua_State *L)
{
...
static int
ngx_stream_lua_ngx_worker_count(lua_State *L)
{
    ngx_core_conf_t   *ccf;
