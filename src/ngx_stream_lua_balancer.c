...
ngx_stream_lua_balancer_get_peer(ngx_peer_connection_t *pc, void *data)
{
    ...
    rc = lscf->balancer.handler(s, pc->log, lscf, L);
    ...
    const u_char *adds, size_t addr_len, int port, char **err)
{
    ...
    ngx_memcpy(url.url.data, adds, addr_len);
    ...
    char **err)
{
    ...
    if (!bp->cloned_upstream_conf) {
        /* we clone the upstream conf for the current session so that
         * we do not affect other sessions at all. */

        ucf = ngx_palloc(s->pool, sizeof(ngx_stream_upstream_conf_t));
        ...
        ngx_memcpy(ucf, u->conf, sizeof(ngx_stream_upstream_conf_t));
    ...
    int *status, char **err)
{
   ...
   */
    ...
    /*if (r->upstream_states && s->upstream_states->nelts > 1) {
