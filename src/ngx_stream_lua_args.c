...
static int ngx_stream_lua_ngx_req_set_uri_args(lua_State *L);
static int ngx_stream_lua_ngx_req_get_uri_args(lua_State *L);
static int ngx_stream_lua_ngx_req_get_post_args(lua_State *L);


static int
ngx_stream_lua_ngx_req_set_uri_args(lua_State *L)
{
    ...
    case LUA_TTABLE:
        ngx_stream_lua_process_args_option(r, L, 1, &args);
...
static int
ngx_stream_lua_ngx_req_get_uri_args(lua_State *L)
{
    ngx_stream_lua_request_t          *r;
    u_char                      *buf;
    u_char                      *last;
    ...
    retval = ngx_stream_lua_parse_args(L, buf, last, max);
...
static int
ngx_stream_lua_ngx_req_get_post_args(lua_State *L)
{
    ngx_stream_lua_request_t          *r;
    u_char                      *buf;
    int                          retval;
...
void
ngx_stream_lua_inject_req_args_api(lua_State *L)
{
    lua_pushcfunction(L, ngx_stream_lua_ngx_req_set_uri_args);
    lua_setfield(L, -2, "set_uri_args");

    lua_pushcfunction(L, ngx_stream_lua_ngx_req_get_uri_args);
    lua_setfield(L, -2, "get_uri_args");

    lua_pushcfunction(L, ngx_stream_lua_ngx_req_get_uri_args);
    lua_setfield(L, -2, "get_query_args"); /* deprecated */

    lua_pushcfunction(L, ngx_stream_lua_ngx_req_get_post_args);
...
size_t
ngx_stream_lua_ffi_req_get_querystring_len(ngx_stream_lua_request_t *r)
{
    return r->args.len;
}
...
int
ngx_stream_lua_ffi_req_get_uri_args_count(ngx_stream_lua_request_t *r, int max)
{
    int                      count;
    u_char                  *p, *last;
...
int
ngx_stream_lua_ffi_req_get_uri_args(ngx_stream_lua_request_t *r, u_char *buf,
    ngx_stream_lua_ffi_table_elt_t *out, int count)
{
