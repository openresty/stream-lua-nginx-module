...
static int ngx_stream_lua_ngx_re_gmatch_iterator(lua_State *L);
static ngx_uint_t ngx_stream_lua_ngx_re_parse_opts(lua_State *L,
    ngx_stream_lua_regex_compile_t *re, ngx_str_t *opts, int narg);
static int ngx_stream_lua_ngx_re_sub_helper(lua_State *L, unsigned global);
static int ngx_stream_lua_ngx_re_match_helper(lua_State *L, int wantcaps);
static int ngx_stream_lua_ngx_re_find(lua_State *L);
static int ngx_stream_lua_ngx_re_match(lua_State *L);
static int ngx_stream_lua_ngx_re_gmatch(lua_State *L);
static int ngx_stream_lua_ngx_re_sub(lua_State *L);
...
static int
ngx_stream_lua_ngx_re_match(lua_State *L)
{
    return ngx_stream_lua_ngx_re_match_helper(L, 1 /* want captures */);
}


static int
ngx_stream_lua_ngx_re_find(lua_State *L)
{
    return ngx_stream_lua_ngx_re_match_helper(L, 0 /* want captures */);
}


static int
ngx_stream_lua_ngx_re_match_helper(lua_State *L, int wantcaps)
{
    /* u_char                      *p; */
    int                          res_tb_idx = 0;
    ...
    ngx_memzero(&re_comp, sizeof(ngx_stream_lua_regex_compile_t));
    ...
    flags = ngx_stream_lua_ngx_re_parse_opts(L, &re_comp, &opts, 3);
    ...
    if (flags & NGX_LUA_RE_COMPILE_ONCE) {
        ...
        if (re) {
            ngx_log_debug2(NGX_LOG_DEBUG_STREAM, s->connection->log, 0,
                           "stream lua regex cache hit for match regex "
                           "\"%s\" with options \"%s\"", pat.data, opts.data);
        ...
        ngx_log_debug2(NGX_LOG_DEBUG_STREAM, s->connection->log, 0,
                       "stream lua regex cache miss for match regex \"%s\" "
        ...
        if (lmcf->regex_cache_entries >= lmcf->regex_cache_max_entries) {
            ...
            if (lmcf->regex_cache_entries == lmcf->regex_cache_max_entries) {
                ngx_log_error(NGX_LOG_WARN, s->connection->log, 0,
                              "stream lua exceeding regex cache max "
                              "entries (%i)", lmcf->regex_cache_max_entries);
    ...
    dd("pool %p, s pool %p", pool, s->connection->pool);
    ...
    ngx_log_debug5(NGX_LOG_DEBUG_STREAM, s->connection->log, 0,
                   "stream lua compiling match regex \"%s\" with "
                   "options \"%s\" (compile once: %d) (dfa mode: %d) "
                   "(jit mode: %d)", pat.data, opts.data,
...
#   endif /* NGX_DEBUG */
    ...
    if (flags & NGX_LUA_RE_COMPILE_ONCE) {
        ...
        ngx_log_debug2(NGX_LOG_DEBUG_STREAM, s->connection->log, 0,
                       "stream lua saving compiled regex (%d captures) into "
...
ngx_stream_lua_ngx_re_gmatch(lua_State *L)
{
    ...
    if (flags & NGX_LUA_RE_COMPILE_ONCE) {
        ...
        if (re) {
            ngx_log_debug2(NGX_LOG_DEBUG_STREAM, s->connection->log, 0,
                           "stream lua regex cache hit for match regex \"%s\" "
                           "with options \"%s\"", pat.data, opts.data);
    ...
    ngx_log_debug5(NGX_LOG_DEBUG_STREAM, s->connection->log, 0,
                   "stream lua compiling gmatch regex \"%s\" with "
                   "options \"%s\" (compile once: %d) (dfa mode: %d) "
                   "(jit mode: %d)", pat.data, opts.data,
                   (flags & NGX_LUA_RE_COMPILE_ONCE) != 0,
                   (flags & NGX_LUA_RE_MODE_DFA) != 0,
...
compiled:
    ...
    ctx = lua_newuserdata(L, sizeof(ngx_stream_lua_regex_ctx_t));
    ...
    if (!(flags & NGX_LUA_RE_COMPILE_ONCE)) {
        ...
        lua_pushcfunction(L, ngx_stream_lua_ngx_re_gmatch_gc);
        ...
        cln->handler = ngx_stream_lua_ngx_re_gmatch_cleanup;
        cln->data = ctx;
    ...
    lua_pushcclosure(L, ngx_stream_lua_ngx_re_gmatch_iterator, 3);
...
ngx_stream_lua_ngx_re_gmatch_iterator(lua_State *L)
{
    ...
    ctx = (ngx_stream_lua_regex_ctx_t *) lua_touserdata(L, lua_upvalueindex(2));
    ...
    if (s != ctx->session
        || s->connection->pool != ctx->session->connection->pool)
    {
    ...
    if (rc == NGX_REGEX_NO_MATCHED) {
        ...
        if (!(ctx->flags & NGX_LUA_RE_COMPILE_ONCE)) {
            if (ctx->regex_sd) {
                ngx_stream_lua_regex_free_study_data(s->connection->pool,
                                                     ctx->regex_sd);
                ctx->regex_sd = NULL;
            }

            ngx_pfree(s->connection->pool, cap);
        }
...
static int
ngx_stream_lua_ngx_re_sub(lua_State *L)
{
    return ngx_stream_lua_ngx_re_sub_helper(L, 0 /* global */);
}


static int
ngx_stream_lua_ngx_re_gsub(lua_State *L)
{
    return ngx_stream_lua_ngx_re_sub_helper(L, 1 /* global */);
}
...
ngx_stream_lua_ngx_re_sub_helper(lua_State *L, unsigned global)
{
    ...
    flags = ngx_stream_lua_ngx_re_parse_opts(L, &re_comp, &opts, 4);
    ...
    if (flags & NGX_LUA_RE_COMPILE_ONCE) {
        ...
        if (re) {
            ngx_log_debug3(NGX_LOG_DEBUG_STREAM, s->connection->log, 0,
                           "stream lua regex cache hit for sub regex \"%s\" "
                           "with options \"%s\" and replace \"%s\"",
                           pat.data, opts.data,
        ...
        ngx_log_debug4(NGX_LOG_DEBUG_STREAM, s->connection->log, 0,
                       "stream lua regex cache miss for %ssub regex \"%s\" "
                       "with options \"%s\" and replace \"%s\"",
    ...
    ngx_log_debug6(NGX_LOG_DEBUG_STREAM, s->connection->log, 0,
                   "stream lua compiling %ssub regex \"%s\" with options "
                   "\"%s\" (compile once: %d) (dfa mode: %d) (jit mode: %d)",
                   global ? "g" : "", pat.data, opts.data,
                   (flags & NGX_LUA_RE_COMPILE_ONCE) != 0,
                   (flags & NGX_LUA_RE_MODE_DFA) != 0,
                   (flags & NGX_LUA_RE_MODE_JIT) != 0);
...
#   endif /* NGX_DEBUG */
    ...
    if (func) {
        ...
        ctpl = ngx_palloc(pool, sizeof(ngx_stream_lua_complex_value_t));
        if (ctpl == NULL) {
            flags &= ~NGX_LUA_RE_COMPILE_ONCE;
            msg = "no memory";
            goto error;
        ...
        ccv.pool = pool;
        ccv.log = s->connection->log;
        ccv.value = &tpl;
        ccv.complex_value = ctpl;

        if (ngx_stream_lua_compile_complex_value(&ccv) != NGX_OK) {
            ngx_pfree(pool, cap);
            ngx_pfree(pool, ctpl);
    ...
    if (flags & NGX_LUA_RE_COMPILE_ONCE) {
        ...
        ngx_log_debug2(NGX_LOG_DEBUG_STREAM, s->connection->log, 0,
                       "stream lua saving compiled sub regex (%d captures) "
                       "into the cache (entries %i)", re_comp.captures,
                       lmcf->regex_cache_entries);
...
exec:
    ...
    for (;;) {
        if (flags & NGX_LUA_RE_MODE_DFA) {
            ...
            rc = ngx_stream_lua_regex_exec(re_comp.regex, sd, &subj, offset,
                                           cap, ovecsize, exec_opts);
        }
        ...
        rc = ngx_stream_lua_complex_value(s, &subj, cp_offset, rc, cap, ctpl,
                                          &luabuf);
...
void
ngx_stream_lua_inject_regex_api(lua_State *L)
{
    /* ngx.re */

    lua_createtable(L, 0, 5 /* nrec */);    /* .re */

    lua_pushcfunction(L, ngx_stream_lua_ngx_re_find);
    lua_setfield(L, -2, "find");

    lua_pushcfunction(L, ngx_stream_lua_ngx_re_match);
    lua_setfield(L, -2, "match");

    lua_pushcfunction(L, ngx_stream_lua_ngx_re_gmatch);
    lua_setfield(L, -2, "gmatch");

    lua_pushcfunction(L, ngx_stream_lua_ngx_re_sub);
    lua_setfield(L, -2, "sub");

    lua_pushcfunction(L, ngx_stream_lua_ngx_re_gsub);
...
ngx_stream_lua_ngx_re_gmatch_cleanup(void *data)
{
    ...
    if (ctx) {
        if (ctx->regex_sd) {
            ngx_stream_lua_regex_free_study_data(ctx->session->connection->pool,
                                                 ctx->regex_sd);
            ctx->regex_sd = NULL;
        }
...
ngx_stream_lua_ngx_re_gmatch_gc(lua_State *L)
{
    ...
    if (ctx && ctx->cleanup) {
        ngx_stream_lua_ngx_re_gmatch_cleanup(ctx);
    }
...
static void
ngx_stream_lua_re_collect_named_captures(lua_State *L, int res_tb_idx,
