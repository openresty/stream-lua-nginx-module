...
static uintptr_t ngx_stream_lua_ngx_escape_sql_str(u_char *dst, u_char *src,
        size_t size);
static int ngx_stream_lua_ngx_escape_uri(lua_State *L);
static int ngx_stream_lua_ngx_unescape_uri(lua_State *L);
static int ngx_stream_lua_ngx_quote_sql_str(lua_State *L);
static int ngx_stream_lua_ngx_md5(lua_State *L);
static int ngx_stream_lua_ngx_md5_bin(lua_State *L);

#if (NGX_HAVE_SHA1)
static int ngx_stream_lua_ngx_sha1_bin(lua_State *L);
#endif

static int ngx_stream_lua_ngx_decode_base64(lua_State *L);
...
{
    lua_pushcfunction(L, ngx_stream_lua_ngx_escape_uri);
    ...
    lua_pushcfunction(L, ngx_stream_lua_ngx_decode_base64);
    lua_setfield(L, -2, "decode_base64");

    lua_pushcfunction(L, ngx_stream_lua_ngx_encode_base64);
    lua_setfield(L, -2, "encode_base64");

    lua_pushcfunction(L, ngx_stream_lua_ngx_md5_bin);
    lua_setfield(L, -2, "md5_bin");

    lua_pushcfunction(L, ngx_stream_lua_ngx_md5);
    lua_setfield(L, -2, "md5");

#if (NGX_HAVE_SHA1)
    lua_pushcfunction(L, ngx_stream_lua_ngx_sha1_bin);
    lua_setfield(L, -2, "sha1_bin");
#endif

    lua_pushcfunction(L, ngx_stream_lua_ngx_crc32_short);
    lua_setfield(L, -2, "crc32_short");

    lua_pushcfunction(L, ngx_stream_lua_ngx_crc32_long);
...
static int
ngx_stream_lua_ngx_escape_uri(lua_State *L)
{
    size_t                   len, dlen;
    uintptr_t                escape;
    u_char                  *src, *dst;
...
static int
ngx_stream_lua_ngx_unescape_uri(lua_State *L)
{
    size_t                   len, dlen;
    u_char                  *p;
    u_char                  *src, *dst;
    ...
    ngx_stream_lua_unescape_uri(&dst, &src, len, NGX_UNESCAPE_URI_COMPONENT);
...
static int
ngx_stream_lua_ngx_md5(lua_State *L)
{
    u_char                  *src;
    size_t                   slen;
...
static int
ngx_stream_lua_ngx_md5_bin(lua_State *L)
{
    u_char                  *src;
    size_t                   slen;
...
#if (NGX_HAVE_SHA1)
static int
ngx_stream_lua_ngx_sha1_bin(lua_State *L)
{
    u_char                  *src;
    size_t                   slen;
...
static int
ngx_stream_lua_ngx_decode_base64(lua_State *L)
{
    ngx_str_t                p, src;
...
static size_t
ngx_stream_lua_base64_encoded_length(size_t n, int no_padding)
{
...
static int
ngx_stream_lua_ngx_encode_base64(lua_State *L)
{
    int                      n;
    int                      no_padding = 0;
    ngx_str_t                p, src;
    ...
    p.len = ngx_stream_lua_base64_encoded_length(src.len, no_padding);

    p.data = lua_newuserdata(L, p.len);

    ngx_stream_lua_encode_base64(&p, &src, no_padding);
...
static int
ngx_stream_lua_ngx_crc32_short(lua_State *L)
{
    u_char                  *p;
    size_t                   len;
...
static int
ngx_stream_lua_ngx_crc32_long(lua_State *L)
{
    u_char                  *p;
    size_t                   len;
